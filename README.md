...
The installation and configuration guide has two parts:
...
3. The scenario we are going to deploy requires two applications:
  * A client application (the DNN web application) that will make requests to a Web API
  * And a Web API (the "resource" on the OAuth webflow).
**First of all we will setup the Web API app with permissions to read the AD**, and then setup the second DNN Web app that has permissions to access the first app.
  1. Click on "Add an application my organization is developing"
  2. When the Create page appears, enter your application's registration information:
    * **Name**: This can be any name you want and is simply how you will identify the application in your Azure Active Directory (i.e. "My Contoso WebAPI").
  ...
  3. When finished, click "Create". Azure AD assigns a unique Application ID to your application, and you're taken to your application's main registration page.
  4. Click on the name of the app we've just created and then on "All settings" > "Required permissions" > "Windows Azure Active Directory". Ensure that the app has, at least, "Sign in and read user profile" checked, in the "Delegated permissions" section.
